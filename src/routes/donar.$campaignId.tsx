...
export const Route = createFileRoute("/donar/$campaignId")({
...
function CampaignDetailPage() {
  ...
  useEffect(() => {
    ...
    const q = query(collection(db, "donations"), where("campaignId", "==", campaignId));
...
function HeroSection({
  ...
  const handleShareProgress = async () => {
    ...
    if (pct >= 100) {
      ...
      text = `💪 Llevamos ${sponsored} de ${goal} mochilas apadrinadas en "${campaign.title}" — ¡ya vamos más de la mitad! Solo faltan ${remaining} para llegar a la meta.\n\n¿Nos ayudas a completarla? 🎒\n${shareUrl}`;
    } else {
      text = `🎒 La campaña "${campaign.title}" está en marcha. Ya tenemos ${sponsored} mochilas apadrinadas y nos faltan ${remaining}. ¡Cada aportación cuenta!\n\nÚneteles en: ${shareUrl}`;
    }
    ...
    <section ref={ref} className="bg-hero-gradient relative overflow-hidden">
      ...
      <div className="container-tight relative grid lg:grid-cols-[1fr_400px] gap-14 items-center py-20 sm:py-28">
        ...
        <div className={cn(
          ...
          <img
            src={posterImg}
            alt={`Afiche de la campaña ${campaign.title}`}
...
function CampaignFooter() {
  const share = () => {
    ...
    if (navigator.share) {
      navigator.share({ title: "Eres Clave", text, url: window.location.href });
    } else {
      ...
      <div className="container-tight flex flex-col sm:flex-row items-start sm:items-center justify-between gap-8">
        <div>
          ...
          <h2 className="text-2xl sm:text-3xl font-black text-white tracking-tight max-w-sm leading-tight">
            Comparte y lleguemos a la meta.
          </h2>
        ...
        <div className="px-6 py-6">
          ...
          <div className="mt-6 flex flex-col gap-3">
            <button
              onClick={() => {
                const msgs = [
                  `Acabo de apadrinar la mochila #${backpack.id} para los niños de Las Charcas. ¡Únete a Eres Clave y apoya la educación! 🎒✨ ${window.location.href}`,
                  `¡Ya puse mi granito de arena! Apadriné la mochila #${backpack.id} para la juventud de Las Charcas. Hazlo tú también: ${window.location.href} 🚀`,
                  `La educación cambia vidas. Acabo de apadrinar la mochila #${backpack.id} en Eres Clave. ¡Súmate a esta iniciativa! ❤️ ${window.location.href}`
                ];
                const text = msgs[Math.floor(Math.random() * msgs.length)];
                if (navigator.share) {
                  navigator.share({ title: "Eres Clave - Las Charcas", text }).catch(() => {
                    navigator.clipboard.writeText(text);
